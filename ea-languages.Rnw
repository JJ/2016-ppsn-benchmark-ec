...
\title{An examination of the speed of evolutionary algorithms in
  different programming languages} 

%PABLO: in conference mode this is how to create the authors
\author{Juan-Julián Merelo-Guervós, Israel Blancas-Álvarez, Pedro
 A. Castillo, Gustavo Romero, Antonio Fernández-Ares \inst{1}
\and Victor M. Rivas
\and Mario García-Valdez, Amaury Hernández-Águila
\and Mario Román }

\institute{CITIC and Computer Architecture and Technology
  Department\\
...
\begin{abstract}
  It is not usual in the evolutionary algorithms area to benchmark
different operations in order to choose the best language for a single
or multilanguage implementation, relying instead on common practice or
on usual frameworks using mainstream languages. % maybe "or on usual
frameworks used in mainstream languages" That is why it is usual to
rely on compiled languages (namely Java or C/C++) when implementing
evolutionary algorithms, without considering other languages or
rejecting them outright on the basis of performance. Since there are a
myriad of languages nowadays, we considered an interesting challenge
to measure their speed when performing usual operations in
...
also make them to choose languages outside the ones mentioned above. Besides,
...
However, in the same spirit of the {\em There is no free lunch} theorem
\cite{Wolpert-1997-NFL} we could affirm there is a {\em no fast lunch}
...
languages. %Pablo: citation to the specific paper that says this?
...
\cite{garcia2010distributed,DBLP:journals/soco/Garcia-SanchezGCAG13} %Pablo: oh, good references! BTW, I changes web-services to service-oriented (more general)
where we could envision that different parts of an application might be written in different
%Pablo: "an application" or maybe better, "an EA"?
...
on implementation are based on facts strictly related to EAs, instead of relying on common lore.
...
of EA operations or the most time-consuming \cite{nesmachnow2011time}, but are representative
